(function () {
  ...
  function buildProductCard(p) {
    ...
    if (dosages.length >= 1) {
      ...
      dosages.forEach(function (d, idx) {
        ...
        var pillHtml = (isActive ? '<svg class="pa-pill-star" viewBox="0 0 12 12" width="10" height="10" fill="currentColor"><path d="M6 1l1.4 2.8L11 4.3l-2.5 2.4.6 3.4L6 8.5 2.9 10.1l.6-3.4L1 4.3l3.6-.5z"/></svg>' : '') + escHtml(d.label);
  ...
  function renderDetailDosageGrid() {
    ...
    dosages.forEach(function(d, idx) {
      ...
      if (isActive) {
        labelSpan.innerHTML = '<svg class="pa-pill-star" viewBox="0 0 12 12" width="10" height="10" fill="currentColor" style="margin-right:3px"><path d="M6 1l1.4 2.8L11 4.3l-2.5 2.4.6 3.4L6 8.5 2.9 10.1l.6-3.4L1 4.3l3.6-.5z"/></svg>' + escHtml(d.label);
      } else {
  ...
  function renderDetailVendors(vendors) {
    ...
    var dosages = state.detailDosages;
    var activeIdx = state.detailActiveDosage;
    var dosageLabel = (dosages[activeIdx] && dosages[activeIdx].label) ? dosages[activeIdx].label : '';
